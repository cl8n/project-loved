	...
	for (const gameMode of Ruleset.all()) {
		...
		for (const nomination of nominationsForMode) {
			...
			mainTopicBeatmapsets.push(
				textFromTemplate(mainTopicBeatmapsetTemplate, {
	...
	await Promise.all(
		nominations.map((nomination) => {
			if (nomination.beatmapset.bgPath != null) {
				return storeTopicCover(nomination.beatmapset.bgPath, nominationTopicIds[nomination.id]);
			}
		}),
	);
}
...
async function generateNews(newsPath, roundInfo, topicIds) {
	...
	for (const gameMode of Ruleset.all()) {
		...
		for (const nomination of nominationsForMode) {
			...
			if (errors.length > 0) {
				console.error(
					chalk.yellow(`Skipping nomination #${nomination.id} with ${joinList(errors)}`),
				);
				continue;
			}

			nominationStrings.push(
				textFromTemplate(newsNominationTemplate, {
					BEATMAPSET: escapeMarkdown(
		...
		gameModeSectionStrings.push(
			textFromTemplate(newsGameModeTemplate, {
				ALL_CAPTAINS: joinList(
					extraInfo.nominators.map(
						(n) => `[${escapeMarkdown(n.name)}](https://osu.ppy.sh/users/${n.id})`,
					),
				),
				CONSISTENT_CAPTAIN:
...
function getExtraBeatmapsetInfo(nomination) {
	const beatmaps = [];
	const beatmapsForMode = nomination.beatmaps.filter(
		(b) => b.game_mode === nomination.game_mode.id,
	);
